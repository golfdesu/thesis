...
This document outlines three high-impact architectural proposals to overcome the limitations of current Transformer-based models for EV charging load forecasting. These proposals are synthesized from the gap analysis of all 118 ingested research papers (see [[research_gaps]] and [[index]]).
...
> [!NOTE] **Global Novelty Verified (re-checked 2026-08-26 against all 118 local papers)**
> - The full re-ingestion (118 papers in `wiki/papers/`) still confirms **0 overlap** between the Mamba backbone and the PICNN/Conformalized Quantile frameworks: [[2026_Mamba_3_Sequence_Modeling]], [[2026_Mamba_KAN_HyKANet_EV]], [[2026_PC_M3_Mamba_EV_Clusters]], plus the foundational TimeMachine ([[2024_TimeMachine_Mamba_Long_Term_Forecasting]]) and Bi-Mamba+ ([[2024_BiMamba_Bidirectional_Mamba_Forecasting]]) lines are all **point-forecast** Mamba/SSM models with no probabilistic heads, while [[2025_Coherent_Hierarchical_EV_Load]] (PICNN) and [[2024_Conformal_Prediction_DER]] (hierarchical conformal) use no Mamba/SSM backbones.
> - **New nearest neighbors from papers 81–103 (each narrows the claim but misses ≥2 of the 3 components):**
>   - [[2026_TFT_Conformal_Environmental_EV_Load]] — Transformer + quantile head + **post-hoc CQR conformal calibration** for EV load. Backbone is TFT (LSTM-attention), calibration is a *single static validation split* (no adaptivity under shift → Gap P-3 open), quantile head is a plain linear layer (**no PICNN monotonicity** → Gap P-1 open). No Mamba, no cross-attention exogenous fusion.
>   - [[2026_DualDirection_Transformer_EV_Charging]] (USDT) — native probabilistic Transformer (per-step Gaussian + CRPS) on Informer/Probformer backbone. No Mamba, no coverage guarantees, covariates limited to calendar/station IDs.
>   - [[2026_MetaLearning_Informer_Probabilistic_EV]] (MAML-Informer) — probabilistic few-shot per-station forecasting with PICP/CRPS/Winkler. Plain quantile heads: no conformal layer, no convexity constraint, no Mamba.
> - Caveat refined: the corpus now contains **Mamba + attention hybrids** (HyKANet's dynamic adjacency module; MFT's scale-masked cross-attention in [[2026_MFT_Multi_Scale_Fusion_Transformer]]) — so *Mamba + cross-attention alone* is no longer claimable as novel. Likewise "first probabilistic/conformalized/dynamic-graph EV Transformer" claims are now refuted by USDT / MoghadamDost / EVformer ([[2026_EVformer_Spatio_Temporal_Decoupled_Citywide]]) respectively.
> - **What remains unexplored (precisely):** the full combination — **Mamba backbone + cross-attention exogenous fusion + conformalized PICNN head** — i.e., an $O(N)$ SSM encoder fused with external drivers via attention, producing *monotonic* quantiles with *distribution-free* coverage that *adaptively recalibrates* under non-stationarity. No paper in the 118-paper corpus implements or ablates this combination; it remains the thesis's defensible novelty per Gaps 6 / T-7 / P-1..P-5 in [[research_gaps]].
...
- **Corpus basis:** [[2026_Mamba_KAN_HyKANet_EV]] shows Mamba + KAN learnable activations beating attention-based PAG by ~17% RMSE for EV demand.
...
- **Corpus basis:** [[2026_PC_M3_Mamba_EV_Clusters]] runs physics-constrained Mamba in MIMO mode over 10,000+ EV clusters in real time; [[2025_Benchmark_Foundation_Models]] shows foundation models (Chronos, TimesFM) competitive only below ~4 weeks of history.
...
- **Corpus basis:** [[2026_ML_Comparison_EV_Charging_Forecasting]] shows in a reproducible 4-city protocol that Transformers win only short-term horizons while GRU/LSTM win mid/long-term; [[2026_ML_Geographical_Transferability_EV]] shows linear SGD matching DL at station level with performance plateauing after ~3 days of data; [[2026_Similar_Day_Selection_EV_Load]] provides a competitive non-DL hybrid (similar-day selection + NSGA-II).
...
- **Relation to flagship:** This is a *defensive* evaluation gate for the winning formula; it also supplies the low-data axis (1/3/7/14 days) for the few-shot ablations informed by [[2025_MixerInformer_Transfer_Learning_New_EV_Stations]] and [[2026_MetaLearning_Informer_Probabilistic_EV]].
