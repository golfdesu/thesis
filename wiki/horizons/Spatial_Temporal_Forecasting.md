...
|-------|-------------|----------------|
| Station-level | Forecast each station independently | [[2019_EV_Load_Forecasting]] |
| Multi-station graph | Stations as graph nodes with adjacency edges | [[2022_GCN_TRN_EV_Availability]], [[2025_MSSTGAN_City_EV_Load]] |
| Feeder/district | Aggregate at distribution network level | [[2021_Hierarchical_Probabilistic_EV_Load]] |
| City-wide | All public stations in a metropolitan area | [[2025_EV_STLLM_Spatio_Temporal_LLM]] |
| Regional (physics-informed) | Road network + grid topology | [[2024_Physics_Informed_GAT_EV_Load]] |
...
- **Price misinterpretation**: Naive models incorrectly learn positive demand-price correlations during peak pricing (spurious correlation). See [[2024_Physics_Informed_GAT_EV_Load]].

## Common Model Approaches

| Model | Spatial Component | Temporal Component | Papers |
|-------|------------------|--------------------|--------|
| GCN-TRN | Graph Convolution | Transformer | [[2022_GCN_TRN_EV_Availability]] |
| Graph WaveNet | Adaptive-adjacency + bidirectional diffusion GCN | Gated dilated causal TCN | [[2019_Graph_WaveNet_Spatial_Temporal_Modeling]] |
| DiffSTG / UGnet | Vanilla GCN inside Unet denoiser | Masked DDPM diffusion (non-autoregressive) | [[2023_DiffSTG_Probabilistic_ST_Graph_Diffusion]] |
| PAG (Physics-Informed) | GAT + PIML | TPA-LSTM | [[2024_Physics_Informed_GAT_EV_Load]] |
| MSSTGAN | Pyramid Split GAT | Adaptive Gated Fusion | [[2025_MSSTGAN_City_EV_Load]] |
| EV-STLLM | Graph Transformer | LLM (LoRA) | [[2025_EV_STLLM_Spatio_Temporal_LLM]] |
| VFL + EGAT-LSTM | Vertical Federated GAT | LSTM | [[2025_Vertical_Federated_EGAT_LSTM]] |
| Multi-Branch ResTrans | ResNet (spatial) | Transformer | [[2023_Multi_Branch_ResTrans_Solar]] |
| Multi-View Graph Contrastive | Mask-Attention Graph Transformer | Time encoder | [[2025_Multi_View_Graph_Intrusion_Detection_EV]] |
| EVformer | Semantics-aware Top-K spatial attention (physical topology × cosine semantic similarity, $O(N^2)\to O(K^2)$) | Channel-first global-aggregation temporal attention + Bidirectional Temporal Bridge | [[2026_EVformer_Spatio_Temporal_Decoupled_Citywide]] |

## Citing Literature

- 2019 — [[2019_Graph_WaveNet_Spatial_Temporal_Modeling]] : Foundational adaptive-adjacency STGNN — learns hidden inter-node dependencies ($\tilde{A}_{adp} = \text{SoftMax}(\text{ReLU}(E_1E_2^T))$) instead of relying on given graphs; SOTA on METR-LA/PEMS-BAY and the backbone reused by EV station GNN forecasters ([[2024_Attention_Spatiotemporal_MultiGraph_EV_Load]], [[2025_MSSTGAN_City_EV_Load]]).
- 2023 — [[2023_DiffSTG_Probabilistic_ST_Graph_Diffusion]] : First DDPM generalized to spatio-temporal graphs — probabilistic multi-station forecasting with uncertainty bands (CRPS −4.3% to −14.3% vs TimeGrad/CSDI/MC Dropout), the machinery needed when downstream grid decisions require distributions, not point forecasts.
...
- [[2021_Hierarchical_Probabilistic_EV_Load]] — Ensemble methodology for hierarchical coherence
- [[2025_Coherent_Hierarchical_EV_Load]] — PICNN + DCL for probabilistic coherent reconciliation
- [[2024_Conformal_Prediction_DER]] — Distribution-free conformal prediction for DER hierarchies
