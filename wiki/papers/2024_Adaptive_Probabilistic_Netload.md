...
**1. Great Britain regional net-load** (extends [[2021_Regional_Netload]] dataset from 2014–2018 to **2014–2021**, including COVID):
...
- Benchmark comparison against APLF ([[2021_APLF_Adaptive_Probabilistic_Load]]) in supplementary material.
...
- [[2021_Regional_Netload]] [21] — base GB regional net-load model (GAM + QR + generalized Pareto tails) extended here to the adaptive setting and to 2021 data.
- [[2021_Conformal_Time_Series_Forecasting]] — same conformal/adaptive-calibration research line; the paper's quantile-recalibration approach parallels Zaffran2022_Adaptive_Conformal [20] (adaptive conformal inference for time series), which motivated using BOA to tune the quantile update online.
- [[2025_QR_LSTM_Attention_EV_Load]] — related use of quantile regression on residuals of a mean model for probabilistic load forecasting (here: linear QR on GAM/Kalman residuals; there: QR-LSTM for EV load).
- [[2021_Kalman_COVID_France]] [10] and [[2022_State_Space_PostCOVID]] [11] — prior Kalman-filter adaptive point load forecasting that this work extends to probabilistic settings.
- [[2021_APLF_Adaptive_Probabilistic_Load]] [12] — adaptive probabilistic benchmark (hidden Markov models, Gaussian predictive distributions; poor tail calibration here).
