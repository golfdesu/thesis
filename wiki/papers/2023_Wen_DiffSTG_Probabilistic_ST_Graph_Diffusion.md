...
- Extends DDPM (Ho et al., NeurIPS 2020) to graphs; conditions like [[2021_Tashiro_CSDI_Conditional_Diffusion_Forecasting]] (CSDI, ref [36]) but adds spatial conditioning + non-autoregressive generation; compares against TimeGrad (Rasul et al., ICML 2021), DeepAR, Latent ODE, MC Dropout (Wu et al., KDD 2021).
- Contrasts WaveNet-based denoising networks with [[2019_Wu_Graph_WaveNet_Spatial_Temporal_Modeling]] (cited as ref [44]); deterministic baselines include DCRNN (Li et al., ICLR 2018) and STGCN (Yu et al., IJCAI 2018).
- Related vault context: probabilistic uncertainty quantification complements deterministic EV station models [[2024_Shi_Attention_Spatiotemporal_MultiGraph_EV_Load]], [[2024_Qu_Physics_Informed_GAT_EV_Load]]; diffusion-based load forecasting lineage continues toward [[2026_Hao_Mamba_KAN_HyKANet_EV]]-era architectures.
