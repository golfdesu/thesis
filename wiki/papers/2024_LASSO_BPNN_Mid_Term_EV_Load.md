...
- **[3] Buzna et al., SyNERGY MED 2019** — time series vs ML comparison for EV load (cf. vault [[2021_Hierarchical_Probabilistic_EV_Load]]).
- **[4] Dabbaghjamanesh et al.** — Q-learning EVCS load forecasting under scenarios (cf. vault [[2021_RL_Q_Learning_EV_Load]]; note the paper's reference list attributes this work differently than the introduction text).
- **[5] Zhu et al., Energies 2019** — deep learning comparative study; LSTM best for super-short-term (cf. vault [[2019_EV_Load_Forecasting]]).
- **[1] Zhang et al., Applied Energy 2020**; **[2] de Quevedo et al., IEEE TSG 2017**; **Zhou et al., Energies 2022** (Bayesian LSTM uncertainty).
- Related vault pages: model counterparts [[XGBoost]] and [[MLP]] (BPNN = MLP-family network trained by back-propagation; XGBoost here is an overfitting-prone baseline), [[SVR]]. Related papers: [[2024_Data_Driven_EVCS_Demand_Forecasting]] (short-term counterpart using MLP/SVR/BRT on public ACN data), [[2024_TOU_Price_Meteorology_EV_Charging_Load]] (alternative exogenous-feature selection route via Pearson/MIC rather than LASSO).
